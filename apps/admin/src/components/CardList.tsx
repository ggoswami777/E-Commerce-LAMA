...
const CardList = async({ title }: { title: string }) => {
  ...
  try {
    ...
    if(title==="Popular Products"){
      const res = await fetch(`${process.env.NEXT_PUBLIC_PRODUCT_SERVICE_URL}/products?limit=4&popular=true`);
      if (res.ok) {
        ...
        console.error(`Failed to fetch products in CardList: ${res.statusText}`);
      }
    }else{
      ...
      if (res.ok) {
        ...
        console.error(`Failed to fetch orders in CardList: ${res.statusText}`);
      }
    }
  } catch (error) {
    console.error(`Error in CardList fetching for title "${title}":`, error);
  }
  return (
    <div className="">
